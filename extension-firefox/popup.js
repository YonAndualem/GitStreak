document.addEventListener('DOMContentLoaded', () => {
    ...
    settingsBtn.addEventListener('click', () => {
        chrome.storage.local.remove([
            ...
            chrome.action.setBadgeText({ text: '' }); // Clear badge on logout
            showSetupScreen();
        });
    });
        ...
        fetch(jsonUrl)
            ...
            .then(data => {
                if (data.heatmapDays) {
                    ...
                    // Force an instant refresh of the badge on the extension icon
                    chrome.action.setBadgeText({ text: data.stats.currentStreak.toString() });
                    chrome.action.setBadgeBackgroundColor({ 
                        color: data.stats.hasCommittedToday ? '#39d353' : '#da3633' 
                    });
                }
            })
            .catch(err => {
